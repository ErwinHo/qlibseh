 ...
 *  - (int) iopause return now the result of poll() or select
 *  - the return code shall be not-negative; and to be evaluated on call
*/
...
int iopause(iopause_fd *x,unsigned int len,struct taia *deadline,struct taia *stamp)
{
  ...
  if (r = select(nfds,&rfds,&wfds,(fd_set *) 0,&tv) < 0)
    return r;
